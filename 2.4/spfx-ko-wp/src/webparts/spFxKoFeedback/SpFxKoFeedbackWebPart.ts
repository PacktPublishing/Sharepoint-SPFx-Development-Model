...
import * as strings from 'spFxKoFeedbackStrings';
import SpFxKoFeedbackViewModel, { ISpFxKoFeedbackBindingContext } from './SpFxKoFeedbackViewModel';
import { ISpFxKoFeedbackWebPartProps } from './ISpFxKoFeedbackWebPartProps';

let _instance: number = 0;

export default class SpFxKoFeedbackWebPart extends BaseClientSideWebPart<ISpFxKoFeedbackWebPartProps> {
  private _id: number;
  ...
  protected onInit(): Promise<void> {
    ...
    const bindings: ISpFxKoFeedbackBindingContext = {
  ...
  private _registerComponent(tagName: string): void {
    ko.components.register(
      tagName,
      {
        viewModel: SpFxKoFeedbackViewModel,
        template: require('./SpFxKoFeedback.template.html'),
        synchronous: false
      }
    );
  }
